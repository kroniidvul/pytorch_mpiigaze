...
def train(epoch, model, optimizer, criterion, train_loader, config, writer):
    ...
    if config['tensorboard']:
        writer.add_scalar('Train/Loss.{}'.format(args.test_id), loss_meter.avg, epoch)
...
#        print( 'im shape: ' , images.shape)
    ...
    if config['tensorboard']:
        if epoch > 0:
            writer.add_scalar('Test/Loss{}'.format(args.test_id), loss_meter.avg, epoch)
...
#    for param in model.vgg_1.parameters():
#        param.require_grad = False
#        
#    for param in model.vgg_2.parameters():
#        param.require_grad = False
#        
#    for param in model.vgg_3.parameters():
#        param.require_grad = True
#
#    for param in model.vgg_4.parameters():
#        param.require_grad = True
...
#        optimizer, base_lr=1e-6, max_lr=0.1)
